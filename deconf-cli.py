...
def get_api_key(gw):
    ...
    response = requests.post('http://{}:{}/api'.format(gw.ip, gw.port), data = json.dumps({"devicetype":name}))
    if response.status_code == requests.codes.ok:
...
def purge_json():
    confirm = input('Are you sure you want to remove locally saved gateway information? (y/n)')
    if confirm == 'y':
        print('Purged')
...
def save_gateway_info(gw):
    ...
    gw.state_ok = True
    print('Success! Saved gateway information and API key {} to {}'.format(gw.key, gw.filename))

def list_entities(gw):
    try:
        response = requests.get('http://{}:{}/api/{}/sensors'.format(gw.ip, gw.port, gw.key))
    except requests.ConnectionError:
        print('Connection error')
    if response.status_code == requests.codes.ok:
    ...
    for i in gw.configurables:
        if i['id'] == id:
            if i['type'] == "ZHAPresence":
                new = input('Current duration {}. Enter new value: '.format(i['duration']))
                response = requests.put('http://{}:{}/api/{}/sensors/{}/config'.format(gw.ip, gw.port, gw.key, id), data = json.dumps({'duration': new}))
                if response.status_code == requests.codes.ok:
                    print('Success!')
                    return
            elif i['type'] == "ZHAVibration":
                new = input('Current sensitivity {}. Enter new value: '.format(i['sensitivity']))
                response = requests.put('http://{}:{}/api/{}/sensors/{}/config'.format(gw.ip, gw.port, gw.key, id), data = json.dumps({'sensitivity': new}))
                if response.status_code == requests.codes.ok:
                    print('Success!')
                    return
